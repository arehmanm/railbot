...
def callback(val):
    ...
    img1 = cv2.imread("../output/output" + str(3*n) + ".png")
    img2 = cv2.imread("../output/output" + str(3*n+1) + ".png")
    img3 = cv2.imread("../output/output" + str(3*n+2) + ".png")
    ...
    multiplier = cv2.getTrackbarPos('Mul', 'Control')
    ...
    center3 = (multiplier*offset[0]+center2[0], offset[1]+center2[1])
    center3 = tuple(center3)
    ...
    img = numpy.concatenate((imgc1, imgc2, imgc3), axis=1)
...
cv2.createTrackbar("Mul", "Control", 1, 10, callback); #Hue (0 - 179)
